 ...
 * a set of base dimensions, and all other dimensions (called derived
 * dimensions) are expressed as the product of base dimensions raised to some
 * power.
 *
 ...
 *
 * This class stores an array of base dimension powers. The number of base
 * dimensions is specified as a template argument. Additinally, the type to use
 * for dimension powers can be specified.
 *
 * This allows the user to use a smaller type to build a unit system if they are
 * concerned about memory. i.e.
 *
 * sizeof(basic_dimension<3,long int>) > sizeof(basic_dimension<2,long int>) >
 * sizeof(basic_dimension<2,int>)
 *
 * By default, `int` is used to store powers, which should be sufficient for
 * most uses. However, if extremely large powers are expected (I'm not sure why
 * they would be...), a larger type can be used, or the user can use any smaller
 * type capable of representing the powers they need.
 *
 * Physicsl dimensions can be multiplied and devided, so the `operator*(...)`
 * and `operator/(...)` functions are implemented. The compound assignment
 * versions, `operator*=(...)` and `operator/=(...)` are also implemented. These
 * turn out to be useful when building up dimensions while parsing a string
 * representing some unit or dimension relationship.
 */
...
class basic_dimension
{
 public:
  ...
  bool is_base() const
  {
    auto count = std::accumulate(m_Powers.begin(), m_Powers.end(), 0,
                                 [](exponent_type sum, exponent_type power) {
                                   return std::move(sum) + abs(power);
                                 });
    return count == 1 || count == 0;
  }
  bool is_derived() const { return !this->is_base(); }
  bool is_dimensionless() const
  {
    return !std::any_of(m_Powers.begin(), m_Powers.end(),
                        [](exponent_type p) { return p != 0; });
  }

  const array_type& powers() const { return m_Powers; }
