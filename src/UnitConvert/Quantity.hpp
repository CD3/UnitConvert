...
class Quantity
{
  ...
  public:
    ...
    UnitRegistry const* const
      unitRegistry() const;

    template<typename TT>
      friend std::ostream&
      operator<<(std::ostream& out, const Quantity<TT>& quant);

    Quantity<T>
      to(const Unit& unit) const;
...
template <typename T>
const T&
Quantity<T>:: value() const
{
  return m_Value;
}
template <typename T>
const Unit&
Quantity<T>::unit() const
{
  return m_Unit;
}
template <typename T>
...
Quantity<T>::to() const
{
  return boost::units::quantity<UNIT, TT>::from_value(
      this->to(detail::str(UNIT{}, boost::units::format_mode::raw_fmt)).value());
}
