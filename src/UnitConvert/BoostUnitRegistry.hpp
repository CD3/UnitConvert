...
struct addSystemBaseUnits {
  static void apply(UnitRegistry* ureg)
  {
...
/**
 * A unit registry that integrates with Boost.Units. The template parameter
 * SYSTEM specifies a Boost.Units system that will be use to automatically load
...
class BoostUnitRegistry : public UnitRegistry
{
 public:
  ...
  void addUnit(const boost::units::unit<D, S>& u)
  {
    ...
    si_base_unit su;
    this->addUnit(UnitConvert::detail::str(u),
                  boost::units::conversion_factor(u, su) *
                      this->makeUnit(UnitConvert::detail::str(
                          su, boost::units::format_mode::raw_fmt)));
  }
};
